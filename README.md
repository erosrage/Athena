...
flowchart TD
    ...
    NP1 --> CC["Open Claude Code — full interactive session"]:::llm
    EP1 --> CC
    CC --> CH["Developer chats freely — Claude asks questions,\nexplores problem, recommends stacks"]:::llm
    CH --> WP["Claude writes PLAN.md via Write tool"]:::llm
    WP --> EX["Developer exits session"]:::action
    EX --> MX{"Mode?"}:::decision
    MX -->|"new project"| MS["Multi-stack picker\nEnter stack numbers comma-separated"]:::prompt
    MS --> SN["Name each service\ne.g. my-app-api, my-app-web"]:::prompt
    SN --> SC["proj new per service\n--stack and --cloud pre-filled"]:::action
    SC --> Z["Done"]:::done
    MX -->|"existing project"| SM{"Story method"}:::decision
    SM -->|"1 — extract via Claude"| H["claude -p extracts stories from PLAN.md"]:::llm
